...
public class AlchemistsToolkit extends Artifact {
  ...
  public AlchemistsToolkit() {
    ...
    for (int i = 1; i <= 3; i++) {
      String potion;
      do {
        potion = convertName(Generator.POTION.INSTANCE.generate().getClass()
                .getSimpleName());
  ...
  public void guessBrew() {
    ...
    if (score == 0) {
      ...
      if (level() == 10) {
        ...
        GLog.p("The mixture you've created seems perfect, you don't think " +
  ...
  @Override
  public String desc() {
    String result = "This toolkit contains a number of regents and herbs used" +
            " to improve the process of " +
            "cooking potions.\n\n";

    if (isEquipped(Dungeon.hero))
      if (cursed)
        result += "The cursed toolkit has bound itself to your side, and " +
                "refuses to let you use alchemy.\n\n";
      else
        result += "The toolkit rests on your hip, the various tools inside " +
                "make a light jingling sound as you move.\n\n";
    ...
    if (level() == 0) {
      ...
      result += "The mixture you have created seems perfect, and the toolkit " +
              "is working at maximum efficiency.";
    } else if (!bstGuess.isEmpty()) {
      result += "Your current best mixture is made from: " + bstGuess.get(0) 
              + ", " + bstGuess.get(1) + ", "
